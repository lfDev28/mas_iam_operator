...
subtitle: "User Guide — `mas-est` v0.1.0-beta.18"
...
| -------------------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | --------------------------------- |
| **MAS IAM operator** | `quay.io/lee_forster/mas-iam-operator:0.0.13` (via catalog `catalog-0.0.13`) | OLM operator that reconciles the `MasIamStack` CR — Keycloak, OpenLDAP, PostgreSQL, SCIM bridge | Yes — reused across installs      |
...
| **SCIM bridge**      | `quay.io/lee_forster/mas-scim-bridge:v0.1.0-beta.18`                         | Polls Keycloak every 5 min, provisions users with prefix `scim.` into MAS                       | Yes                               |
...
| **mas-est CLI**      | `quay.io/lee_forster/mas-external-services-tool:v0.1.0-beta.18`              | Local installer, preflight, status, support-bundle, uninstall                                   | Yes                               |
...
```bash
export MAS_EST_IMAGE='quay.io/lee_forster/mas-external-services-tool:v0.1.0-beta.18'
mkdir -p "$HOME/mas-est"
podman run -ti --rm -v "$HOME/mas-est:/tmp" --pull always "$MAS_EST_IMAGE"
export PATH="$HOME/mas-est:$PATH"
mas-est version    # should report 0.1.0-beta.18
```
