...
subtitle: "User Guide — `mas-est` v0.1.0-beta.23"
...
| **SCIM bridge**      | `quay.io/lee_forster/mas-scim-bridge:v0.1.0-beta.23`                         | Polls Keycloak every 5 min, provisions users with prefix `scim.` into MAS                       | Yes                               |
...
| **mas-est CLI**      | `quay.io/lee_forster/mas-external-services-tool:v0.1.0-beta.23`              | Local installer, preflight, status, support-bundle, uninstall                                   | Yes                               |
...
```bash
export MAS_EST_IMAGE='quay.io/lee_forster/mas-external-services-tool:v0.1.0-beta.23'
mkdir -p "$HOME/mas-est"
podman run -ti --rm -v "$HOME/mas-est:/tmp" --pull always "$MAS_EST_IMAGE"
export PATH="$HOME/mas-est:$PATH"
mas-est version    # should report 0.1.0-beta.23
```
